/*
 TechnoMCU ULTIMATE - Advanced Crane Control System
 - Enhanced JCB Auto Crane Mode with precise servo control
 - Specific movement pattern: X:0° → Y:145° (load) → Y:75° → X:175° → Y:145° (unload)
 - By Raktim Hazra
*/
...
// WiFi AP
const char* apSsid = "TechnoMCU_Crane";
...
int currentMenu = 0;
const char* menuItems[] = {"MAIN", "MANUAL", "SHARPY", "RELAY", "CRANE", "INFO"};
int menuCount = 6;
...
// CRANE Mode Variables - PRECISE MOVEMENT PATTERN
enum CraneState {
  CRANE_IDLE = 0,
  CRANE_MOVE_TO_LOAD_X = 1,      // Move X to 0°
  CRANE_MOVE_TO_LOAD_Y = 2,      // Move Y to 145° (load position)
  CRANE_LOADING = 3,             // Wait 1.5 seconds for loading
  CRANE_LIFT_LOAD = 4,           // Lift to Y:75°
  CRANE_MOVE_TO_UNLOAD_X = 5,    // Move X to 175°
  CRANE_MOVE_TO_UNLOAD_Y = 6,    // Move Y to 145° (unload position)
  CRANE_UNLOADING = 7,           // Wait 1.5 seconds for unloading
  CRANE_RETURN_HOME = 8          // Return to home position
};

CraneState craneState = CRANE_IDLE;
bool craneActive = false;
bool craneAutoCycle = true;
unsigned long craneStateStartTime = 0;

// Crane Positions (as per your requirements)
int craneHomeX = 90;
int craneHomeY = 90;
int craneLoadX = 0;      // X position for loading
int craneLoadY = 145;    // Y position for loading (pick up goods)
int craneLiftY = 75;     // Y position after lifting load
int craneUnloadX = 175;  // X position for unloading
int craneUnloadY = 145;  // Y position for unloading (release goods) - CHANGED TO 145°

// Crane Statistics
int craneLoadsCompleted = 0;
int craneCycleCount = 0;
unsigned long craneOperationStartTime = 0;

// Crane timing
unsigned long craneLoadDuration = 1500;  // 1.5 seconds loading time
unsigned long craneUnloadDuration = 1500; // 1.5 seconds unloading time - CHANGED TO 1.5s

unsigned long lastCraneUpdate = 0;
#define CRANE_UPDATE_INTERVAL 30
...
void updateCraneMode();
void startCraneOperation();
void stopCraneOperation();
void executeCraneState();
void moveToPosition(int targetX, int targetY, int speed);
void updateCraneAnimations();
void saveState();
void loadState();
...
void showBootAnimation(){
  ...
  if(progress < 0.3){
    ...
    for(int y=0; y<revealHeight; y++){
      if(y < 8){
        ssd_printStr(10, y, "CRANE CONTROL");
    ...
    ssd_printStr(15, 2, "CRANE CONTROL");
    ssd_printStr(25, 4, "SYSTEM");
    ...
    ssd_printStr(15, 1, "CRANE CONTROL");
    ssd_printStr(25, 2, "SYSTEM");
...
void showMainDisplay(){
  ...
  ssd_printStr(0, 0, "CRANE CONTROL");
  ...
  int modeX[] = {20, 45, 70, 95};
  for(int i=0; i<4; i++){
    int intensity = (operationMode == i) ? 0xFF : 0x18;
    const char* modeText = (i==0)?"MAN":(i==1)?"SHP":(i==2)?"RLY":"CRN";
...
void showCraneMode() {
  ...
  // Crane Interface
  ssd_printStr(0, 0, "AUTO CRANE MODE");
  
  // Current State Display
  const char* craneStates[] = {
    "IDLE", "MOVE TO LOAD X", "MOVE TO LOAD Y", "LOADING",
    "LIFT LOAD", "MOVE TO UNLOAD X", "MOVE TO UNLOAD Y", "UNLOADING",
    "RETURN HOME"
  };
  
  ssd_printStr(0, 1, craneStates[craneState]);
  
  // Status Indicator
  if (craneActive) {
    ssd_printStr(80, 1, "ACTIVE");
    
    // Animated activity indicator
    int pulse = (currentTime / 200) % 4;
    for (int i = 0; i <= pulse; i++) {
      if (110 + i * 3 < 128) ssd_buf[1 * 128 + 110 + i * 3] = 0xFF;
    }
  } else {
    ssd_printStr(80, 1, "STANDBY");
  }
  
  // Crane Visualization
  drawCraneVisualization();
  
  // Performance Dashboard
  char perfStr[32];
  snprintf(perfStr, sizeof(perfStr), "LOADS COMPLETED:%2d", craneLoadsCompleted);
  ssd_printStr(0, 4, perfStr);
  
  char statsStr[32];
  snprintf(statsStr, sizeof(statsStr), "CYCLE COUNT:%2d", craneCycleCount);
  ssd_printStr(0, 5, statsStr);
  ...
  if (craneActive) {
    if (craneAutoCycle) {
      ssd_printStr(0, 7, "A:MODE B:STOP C:PAUSE");
    } else {
      ssd_printStr(0, 7, "A:MODE B:START C:STEP");
    }
    
    // Operation timer
    unsigned long opTime = (currentTime - craneOperationStartTime) / 1000;
    char timeStr[10];
    snprintf(timeStr, sizeof(timeStr), "T:%02lu:%02lu", (opTime / 60) % 60, opTime % 60);
    ssd_printStr(100, 7, timeStr);
  } else {
    ssd_printStr(0, 7, "A:MODE B:START C:SETUP");
    
    // Ready indicator with blink
    if (currentTime % 1000 < 500) {
      ssd_printStr(100, 7, "READY");
    }
  }
...
void drawCraneVisualization() {
  int baseX = map(servoXpos, 0, 180, 20, 108);
  int baseY = 7;
  int armX = baseX;
  int armY = map(servoYpos, 0, 180, 2, 7);
  
  // Draw crane base
  for (int i = -8; i <= 8; i++) {
    if (baseX + i >= 0 && baseX + i < 128) {
      ssd_buf[baseY * 128 + baseX + i] = 0xFF;
      if (baseY - 1 >= 0) ssd_buf[(baseY - 1) * 128 + baseX + i] = 0x18;
    }
  }
  
  // Draw crane arm
  for (int y = baseY - 1; y >= armY; y--) {
    int progress = (baseY - y) * 100 / (baseY - armY);
    int xOffset = (progress * 5) / 100;
    
    if (armX + xOffset >= 0 && armX + xOffset < 128 && y >= 0 && y < 8) {
      ssd_buf[y * 128 + armX + xOffset] = 0xFF;
    }
  }
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = 0; j <= 1; j++) {
      int hookX = armX + i;
      int hookY = armY + j;
      if (hookX >= 0 && hookX < 128 && hookY >= 0 && hookY < 8) {
        ssd_buf[hookY * 128 + hookX] = 0xAA;
      }
    }
  }
  
  // Draw load and unload positions
  drawCranePositions();
}

void drawCranePositions() {
  // Load Position (Left) - 145°
  int loadX = map(craneLoadX, 0, 180, 20, 108);
  for (int i = -3; i <= 3; i++) {
    for (int j = 0; j <= 1; j++) {
      if (loadX + i >= 0 && loadX + i < 128 && 6 - j >= 0) {
        ssd_buf[(6 - j) * 128 + loadX + i] = 0x18;
      }
    }
  }
  ssd_drawChar(loadX - 2, 6, 'L');
  
  // Unload Position (Right) - NOW ALSO 145°
  int unloadX = map(craneUnloadX, 0, 180, 20, 108);
  for (int i = -3; i <= 3; i++) {
    for (int j = 0; j <= 1; j++) {
      if (unloadX + i >= 0 && unloadX + i < 128 && 6 - j >= 0) {
        ssd_buf[(6 - j) * 128 + unloadX + i] = 0x18;
      }
    }
  }
  ssd_drawChar(unloadX - 2, 6, 'U');
}

// Main OLED update function
void updateOLED(){
...
void setOperationMode(OperationMode newMode){
  ...
  if(newMode != MODE_CRANE && craneActive){
    stopCraneOperation();
  }
  ...
  switch(newMode){
    ...
    case MODE_RELAY:
      ...
      break;
    case MODE_CRANE:
      displayState = STATE_CRANE;
      craneState = CRANE_IDLE;
      break;
  }
...
// CRANE Control System - IMPLEMENTING YOUR SPECIFIC MOVEMENT PATTERN
void startCraneOperation() {
  craneActive = true;
  craneState = CRANE_MOVE_TO_LOAD_X;
  craneOperationStartTime = millis();
  craneStateStartTime = millis();
...
void executeCraneState() {
  ...
  unsigned long now = millis();
  if (now - lastCraneUpdate < CRANE_UPDATE_INTERVAL) return;
  
  lastCraneUpdate = now;
  unsigned long stateDuration = now - craneStateStartTime;
  
  switch (craneState) {
    case CRANE_MOVE_TO_LOAD_X:
      // Move X to 0° position
      moveToPosition(craneLoadX, servoYpos, 3);
      
      if (servoXpos == craneLoadX) {
        craneState = CRANE_MOVE_TO_LOAD_Y;
        craneStateStartTime = now;
      }
      break;
      
    case CRANE_MOVE_TO_LOAD_Y:
      // Move Y to 145° position (load position)
      moveToPosition(servoXpos, craneLoadY, 3);
      
      if (servoYpos == craneLoadY) {
        craneState = CRANE_LOADING;
        craneStateStartTime = now;
        // Activate relay for loading mechanism if needed
        relayState = true;
        applyRelay();
      }
      break;
    ...
    case CRANE_LOADING:
      // Wait for 1.5 seconds at load position
      if (stateDuration >= craneLoadDuration) {
        craneState = CRANE_LIFT_LOAD;
        craneStateStartTime = now;
      }
      break;
      
    case CRANE_LIFT_LOAD:
      // Lift load to Y:75° position
      moveToPosition(servoXpos, craneLiftY, 3);
      
      if (servoYpos == craneLiftY) {
        craneState = CRANE_MOVE_TO_UNLOAD_X;
        craneStateStartTime = now;
      }
      break;
      
    case CRANE_MOVE_TO_UNLOAD_X:
      // Move X to 175° position
      moveToPosition(craneUnloadX, servoYpos, 3);
      
      if (servoXpos == craneUnloadX) {
        craneState = CRANE_MOVE_TO_UNLOAD_Y;
        craneStateStartTime = now;
      }
      break;
      
    case CRANE_MOVE_TO_UNLOAD_Y:
      // Move Y to 145° position (unload position) - CHANGED TO 145°
      moveToPosition(servoXpos, craneUnloadY, 3);
      
      if (servoYpos == craneUnloadY) {
        craneState = CRANE_UNLOADING;
        craneStateStartTime = now;
        // Deactivate relay for unloading
        relayState = false;
        applyRelay();
      }
      break;
      
    case CRANE_UNLOADING:
      // Wait for 1.5 seconds at unload position - CHANGED TO 1.5s
      if (stateDuration >= craneUnloadDuration) {
        craneLoadsCompleted++;
        craneCycleCount++;
        craneState = CRANE_RETURN_HOME;
        craneStateStartTime = now;
      }
      break;
    ...
    case CRANE_RETURN_HOME:
      ...
      moveToPosition(craneHomeX, craneHomeY, 3);
      
      if (servoXpos == craneHomeX && servoYpos == craneHomeY) {
        if (craneAutoCycle) {
          // Start next cycle
          craneState = CRANE_MOVE_TO_LOAD_X;
        } else {
...
void moveToPosition(int targetX, int targetY, int speed) {
  if (servoXpos < targetX) servoXpos = min(servoXpos + speed, targetX);
  else if (servoXpos > targetX) servoXpos = max(servoXpos - speed, targetX);
  
  if (servoYpos < targetY) servoYpos = min(servoYpos + speed, targetY);
  else if (servoYpos > targetY) servoYpos = max(servoYpos - speed, targetY);
}

void updateCraneAnimations() {
  // Simple animation updates if needed
}

// Enhanced Button Handling for Crane Mode
void handleCraneButtons() {
  bool btnA = digitalRead(BTN_A_PIN) == LOW;
  bool btnB = digitalRead(BTN_B_PIN) == LOW;
  bool btnC = digitalRead(BTN_C_PIN) == LOW;
  
  // Button A: Mode cycling
  if (btnA && !btnAPressed) {
    btnAPressed = true;
    OperationMode newMode = (OperationMode)((operationMode + 1) % 4);
  ...
  // Button B: Start/Stop Crane operation
  if (btnB && !btnBPressed) {
    btnBPressed = true;
    if (craneActive) {
      stopCraneOperation();
    ...
    createParticleBurst(118, 0, 5);
  }
  if (!btnB && btnBPressed) {
    btnBPressed = false;
  }
  
  // Button C: Toggle auto cycle
  if (btnC && !btnCPressed) {
    btnCPressed = true;
    craneAutoCycle = !craneAutoCycle;
...
void handleButtons(){
  ...
  if(displayState == STATE_MENU){
    ...
    if(btnA && btnB && !bothButtonsPressed){
      ...
      switch(currentMenu){
        ...
        case 4: setOperationMode(MODE_CRANE); break;
        case 5: displayState = STATE_MAIN; break;
      }
    }
    if(!(btnA && btnB) && bothButtonsPressed){
  ...
  // Rapid mode
  if(btnB && btnBPressed) {
    if(millis() - btnHoldStartTime > 400) {
      rapidMode = true;
...
void saveState(){
  ...
  EEPROM.write(ADDR_RELAY2, relay2State ? 1 : 0);
  ...
  // Save crane data
  EEPROM.write(10, craneLoadsCompleted & 0xFF);
  EEPROM.write(11, (craneLoadsCompleted >> 8) & 0xFF);
  EEPROM.commit();
  EEPROM.end();
}
...
String webpage(){
  ...
  html += "<title>TechnoMCU Crane Control</title>";
  html += "<style>";
  ...
  html += ".crane-active{background:#ffaa00}";
  html += "h2{text-shadow:0 0 10px rgba(0,255,255,0.5)}";
  html += "</style></head><body>";
  
  html += "<div class='top'><h2>TechnoMCU Crane Control System</h2>";
  html += "<div class='small'>By Raktim Hazra | Both Load/Unload at 145° | 1.5s Wait Times</div></div>";
  ...
  html += "'>RELAY</button>";
  html += "<button id='modeCrane' class='btn ";
  html += (operationMode==MODE_CRANE?"mode-active":"");
  html += "'>CRANE</button>";
  ...
  if(operationMode == MODE_CRANE){
    html += "<div style='margin-top:15px'><strong>Crane Control:</strong></div>";
    html += "<button id='craneStart' class='btn ";
    html += (craneActive?"crane-active":"");
    html += "'>";
    html += (craneActive?"STOP CRANE":"START CRANE");
    html += "</button>";
    html += "<div class='small'>Movement Pattern: X:0°→Y:145°(1.5s)→Y:75°→X:175°→Y:145°(1.5s)</div>";
    html += "<div class='small'>Both Load & Unload at 145° with 1.5s wait</div>";
  }
  ...
  else if(operationMode == MODE_RELAY) html += "RELAY";
  else html += "CRANE";
  html += " | X:";
  html += String(servoXpos);
  html += " Y:";
  ...
  if(operationMode == MODE_CRANE){
    html += " | Crane:";
    html += (craneActive?"RUNNING":"STOPPED");
    html += " Loads:";
    html += String(craneLoadsCompleted);
  }
  html += "</div></div>";
  ...
  html += "let statusText='Mode: '+(data.mode==0?'MANUAL':(data.mode==1?'SHARPY':(data.mode==2?'RELAY':'CRANE')))+' | X:'+data.x+' Y:'+data.y+' Relay1:'+(data.relay?'ON':'OFF')+' Relay2:'+(data.relay2?'ON':'OFF');";
  html += "if(data.mode==3){statusText+=' | Crane:'+(data.craneActive?'RUNNING':'STOPPED')+' Loads:'+data.craneCount;}";
  html += "document.getElementById('status').innerHTML=statusText;";
  html += "if(data.mode==3){";
  html += "document.getElementById('craneStart').textContent=data.craneActive?'STOP CRANE':'START CRANE';";
  html += "document.getElementById('craneStart').className=data.craneActive?'btn crane-active':'btn';";
  html += "}";
  html += "});";
  html += "}";
  ...
  html += "if(document.getElementById('craneStart')){";
  html += "document.getElementById('craneStart').onclick=()=>{";
  html += "fetch('/cranetoggle').then(()=>updateStatus());";
  html += "};";
  html += "}";
  ...
  html += "document.getElementById('modeCrane').onclick=()=>{fetch('/setmode?mode=3').then(()=>location.reload());};";
...
void setup(){
  ...
  // Crane control routes
  server.on("/cranetoggle", [](){ 
    if(craneActive){
      stopCraneOperation();
    } else {
      startCraneOperation();
  ...
  server.on("/status", [](){ 
    ...
    json += "\"craneActive\":" + String(craneActive ? 1 : 0) + ",";
    json += "\"craneCount\":" + String(craneLoadsCompleted);
    json += "}";
